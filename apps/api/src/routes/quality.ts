 ...
 * (`requireAuth`, DirectorOnlyError/ScopeDeniedError -> 403). See
 * quality/service.ts's doc for why this endpoint is clinical-scope, not
 * Director-only.
 */
export function createQualityRouter(fhirService: FhirReadService): Router {
  const router = Router();
  router.use(requireAuth);

  router.get('/measures', async (req, res) => {
    try {
      const result = await getDiabetesHba1cMeasure(req.auth!, fhirService);
      res.json(result);
    } catch (err) {
