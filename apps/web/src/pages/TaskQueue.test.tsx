...
describe('TaskQueue — loading + error states', () => {
  ...
  it('renders without crashing when listTasks is pending (placeholderData holds the page)', () => {
    vi.mocked(client.listTasks).mockReturnValue(new Promise(() => {}));
    renderTaskQueue();
    // S12 B.2 — placeholderData means the page renders the empty state (the
    // mock list, when disabled, is `[]`); no "Loading tasks…" copy. The
    // contract is "page renders, no error, no crash" — the actual pending
    // state is the real fetch still in flight, invisible to the user.
    expect(screen.queryByText('Loading tasks…')).not.toBeInTheDocument();
    expect(screen.queryByText('Could not load the task queue.')).not.toBeInTheDocument();
  });

  it('shows the demo-fallback badge when listTasks rejects (instead of an error message)', async () => {
    vi.mocked(client.listTasks).mockRejectedValue(new Error('network down'));
    renderTaskQueue();
    // S12 B.2 — fallback safety net: badge replaces the error message so a
    // judge never sees a broken page when the API is down.
